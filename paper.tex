...
the ranking was checked two further ways. \textbf{Ablation}, which
refits the model with a feature group removed, confirms the reading:
dropping the single 6-hour rolling mean costs $+7.9\,\%$ MAE, more than
dropping all fourteen Open-Meteo exogenous variables together
($+6.3\,\%$). No other individual feature is that expensive. Removing
the temporal cycles costs $+10.0\,\%$, consistent with the third rank of
\texttt{hour\_sin}.

\textbf{Within-regime} analysis addresses the concern that a variable
separating the two measurement windows could rank high merely by acting
as a season indicator. Recomputing the ranking inside each block leaves
6-hour-scale exterior temperature dominant---in the rainy block the
6-hour \emph{lag} of $T_\text{ext}$ leads by a factor of five over the
next feature---so the memory result is not an artefact of the seasonal
contrast.

The same tests temper the negative result on moisture. Exterior
dew-point ranks 20th of 36 in the pooled analysis, yet its rank
\emph{improves} to 6th within the rainy block, which is the opposite of
what a season proxy would do, and removing the five moisture variables
costs $+3.4\,\%$ MAE. Their contribution is therefore small but not
null. Hygroscopic coupling in earthen walls would predict a larger
role, clay conductivity varying with moisture state~\cite{liuzzi2018};
what these data support is a weak effect, not the strong one the
hypothesis implies. Whether that reflects the physics or the limited
...
variables contribute only marginally, so the hygroscopic coupling
hypothesis is not supported at the strength it would predict.
